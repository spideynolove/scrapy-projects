...
def get_time(days: str = None, isfuzzy: bool = False, 
             have_hour: bool=True, oform: str='%Y-%m-%d') -> str:
    hour = ' %H:%M' if have_hour else ''
    return parser.parse(days, fuzzy=isfuzzy).strftime(f'{oform}{hour}') if days else ""

def get_unixtime(timestamp: str = None, divide: int = 1000, have_hour: bool=True) -> str:
    hour = ' %H:%M' if have_hour else ''
